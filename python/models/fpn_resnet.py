...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, scale=1, expansion=5):
        self.inplanes = 64
        ...
        self.conv4 = nn.Conv2d(256, num_classes*2, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        if self.expansion is not None:
            h = F.interpolate(h, scale_factor=self.expansion, mode='bilinear', align_corners=True)
        ...
        cls_logit = self.conv3(out)
        reg_logit = self.conv4(out)
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        pretrained_model = model_zoo.load_url(model_urls['resnet50'], './checkpoints/')
